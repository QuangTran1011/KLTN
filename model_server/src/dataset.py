...
class UserItemRatingDFDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.item_feature is not None:
            item_feature = self.item_feature[idx].toarray().astype(np.float32).squeeze()
        if "tag_embedding" in self.df:
            item_feature = np.hstack([item_feature, self.df["tag_embedding"].iloc[idx].astype(np.float32)])
        return dict(
